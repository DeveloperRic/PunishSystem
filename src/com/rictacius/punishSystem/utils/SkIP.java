...
public class SkIP {
	...
	private static final String IP_INFO_DB_API_KEY = "NONE"; // If you do not
																// setup,
																// IP_INFO_DB
																// will not work
																// !
	...
	public static final SkIPData getIPData(final String ip) {
		try {
			...
			return new SkIPData(json.get(args[0]).toString(), json.get(args[1]).toString(),
					json.get(args[2]).toString(), json.get(args[3]).toString(), json.get(args[4]).toString(),
					Double.valueOf(json.get(args[5]).toString()), Double.valueOf(json.get(args[6]).toString()),
	...
	public static final SkIPDataWeather getWeatherData(final String city, final String countryCode) {
		try {
			final JSONObject weather = (JSONObject) ((JSONArray) ((JSONObject) new JSONParser()
					.parse(httpGet(String.format(OPEN_WEATHER_MAP_URL, city, countryCode)))).get("weather")).get(0);
	...
	public static class SkIPData {
		...
		public SkIPData(final String countryName, final String countryCode, final String region, final String zip,
				final String city, final double latitude, final double longitude, final String timezone) {
	...
	public enum IPMode {
		...
		IP_API("http://ip-api.com/json/%s",
				new String[] { "country", "countryCode", "regionName", "city", "zip", "lat", "lon",
						"timezone" }), IP_INFO_DB(
								"http://api.ipinfodb.com/v3/ip-city/?key=" + IP_INFO_DB_API_KEY + "&ip=%s",
								new String[] { "countryName", "countryCode", "regionName", "cityName", "zipCode",
