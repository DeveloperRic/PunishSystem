...
public class BanCommand implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command arg1, String arg2, String[] args) {
		...
		if (!PermCheck.senderHasAccess(sender, "punishsystem.ban")) {
			sender.sendMessage(ChatColor.RED + "You do not have permission to use the ban command!");
			return true;
		}
		...
		if (!(sender instanceof Player)) {
			...
			GUIBanInput.doTask(toBan, reason, "@Console");
		} else {
			if (args.length < 2) {
				sender.sendMessage(ChatColor.GOLD + "Correct Usage: (/ban <player> <reason>)");
				return true;
			...
			GUIBanInput.doTask(toBan, reason, ((Player) sender).getUniqueId().toString());
		}
		return true;
	}
}
